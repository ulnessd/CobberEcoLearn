...
def load_supervisor(dataset_dir: Path, csv_path: Path):
    ...
    def resolve_path(row):
        if "relative_path" in df.columns and isinstance(row.get("relative_path"), str):
            return dataset_dir / row["relative_path"]
        # fallback: search by label/filename under images
        return dataset_dir / "images" / row["label"] / row["filename"]
